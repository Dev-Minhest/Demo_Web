...
if(class_exists('Vc_Manager')){
        ...
        if(is_array($testimonial)) 
        {
            ...
            foreach ($testimonial as $key => $value) {
                ...
                $html .= '<div class="testimonial-item" data-item="'.$i.'">';
                 $html .= '<strong>'.esc_html__("Image","supershop").' '.$i.':</strong></br>';
                    ...
                    $html .=            '</ul>
                                    ...
                                    <a class="gallery_widget_add_images" href="#" use-single="true" title="'.esc_html__("Add images","supershop").'">'.esc_html__("Add images","supershop").'</a>
                                    <span class="vc_description vc_clearfix">'.esc_html__("Select images from media library.","supershop").'</span>
                                </div>';
                    $html .= '<label>'.esc_html__("Name","supershop").' </label><input style="width:65%;margin-right:10px;margin-bottom:15px" class="st-testimonial" name="'.$i.'[name]" value="'.$value['name'].'" type="text" ></br>';
                    $html .= '<label>'.esc_html__("Position","supershop").' </label><input style="width:65%;margin-right:10px;margin-bottom:15px" class="st-testimonial" name="'.$i.'[pos]" value="'.$value['pos'].'" type="text" ></br>';
                    $html .= '<label>'.esc_html__("Link","supershop").' </label><input style="width:65%;margin-right:10px;margin-bottom:15px" class="st-testimonial" name="'.$i.'[link]" value="'.$value['link'].'" type="text" ></br>';
                    $html .= '<label>'.esc_html__("Description","supershop").' </label><textarea style="width:100%;margin-right:10px;margin-bottom:15px" class="st-testimonial" name="'.$i.'[des]" >'.$value['des'].'</textarea>';
                    $html .= '<a style="color:red" href="#" class="st-del-item">'.esc_html__("Delete","supershop").'</a>';
                $html .= '</div>';
                $i++;
            }
        }
        $html .= '</div>';
        $html .= '<div class="st-add"><button class="vc_btn vc_btn-primary vc_btn-sm st-button-add-testimonial" type="button">'.esc_html__('Add Item', 'supershop').' </button></div>';
    ...
    function sv_add_advantage($settings, $value)
    {
            ...
            foreach ($advantage as $key => $value) {
                ...
                $html .= '<div class="advantage-item" data-item="'.$i.'">';
                 $html .= '<strong>'.esc_html__("Image","supershop").' '.$i.':</strong></br>';
                    ...
                    $html .= '<label>'.esc_html__("Description","supershop").' </label><input style="width:65%;margin-right:10px;margin-bottom:15px" class="st-advantage" name="'.$i.'[des]" value="'.$value['des'].'" type="text" ></br>';
                    $html .= '<label>'.esc_html__("Link","supershop").' </label><input style="width:65%;margin-right:10px;margin-bottom:15px" class="st-advantage" name="'.$i.'[link]" value="'.$value['link'].'" type="text" >';
                    $html .= '<a style="color:red" href="#" class="st-del-item">'.esc_html__("Delete","supershop").'</a>';
                $html .= '</div>';
                $i++;
            }
        ...
        $html .= '</div>';
        $html .= '<div class="st-add"><button class="vc_btn vc_btn-primary vc_btn-sm st-button-add-advantage" type="button">'.esc_html__('Add Item', 'supershop').' </button></div>';
    ...
    function sv_add_brand($settings, $value)
    {
            ...
            foreach ($brand as $key => $value) {
                ...
                $html .= '<div class="brand-item" data-item="'.$i.'">';
                 $html .= '<strong>'.esc_html__("Image","supershop").' '.$i.':</strong></br>';
                    ...
                    $html .= '<label>'.esc_html__("Link","supershop").' </label><input style="width:65%;margin-right:10px;margin-bottom:15px" class="st-brand" name="'.$i.'[link]" value="'.$value['link'].'" type="text" >';
                    $html .= '<a style="color:red" href="#" class="st-del-item">'.esc_html__("Delete","supershop").'</a>';
                $html .= '</div>';
                $i++;
            }
        ...
        $html .= '</div>';
        $html .= '<div class="st-add"><button class="vc_btn vc_btn-primary vc_btn-sm st-button-add-brand" type="button">'.esc_html__('Add Item', 'supershop').' </button></div>';
    ...
    function sv_add_social($settings, $value)
    {
        ...
        if(is_array($social)) 
        {
            ...
            foreach ($social as $key => $value) {
                ...
                $html .= '<div class="social-item" data-item="'.$i.'">';
                    $html .= '<label>'.esc_html__("Social","supershop").' '.$i.':</label></br><label>'.esc_html__("Icon","supershop").' </label><input style="width:65%;margin-right:10px;margin-bottom:15px" class="st-social sv_iconpicker" name="'.$i.'[social]" value="'.$value['social'].'" type="text" ></br>';
                    $html .= '<label>'.esc_html__("Link","supershop").' </label><input style="width:65%;margin-right:10px;margin-bottom:15px" class="st-social" name="'.$i.'[url]" value="'.$value['url'].'" type="text" >';
                    $html .= '<a style="color:red" href="#" class="st-del-item">'.esc_html__("Delete","supershop").'</a>';
                $html .= '</div>';
                $i++;
            }
        ...
        $html .= '</div>';
        $html .= '<div class="st-add"><button class="vc_btn vc_btn-primary vc_btn-sm st-button-add" type="button">'.esc_html__('Add social', 'supershop').' </button></div>';
        ...
        if(is_array($locations)) 
        {
            ...
            foreach ($locations as $key => $value) {
                $html .= '<div class="location-item" data-item="'.$i.'">';
                    $html .= '<div class="wpb_element_label">'.esc_html__("Location",'supershop').' '.$i.'</div>';
                    $html .= '<label>'.esc_html__("Latitude",'supershop').'</label><input class="st-input st-location-save st-title" name="'.$i.'[lat]" value="'.$value['lat'].'" type="text" >';
                    $html .= '<label>'.esc_html__("Longitude",'supershop').'</label><input class="st-input st-location-save st-des" name="'.$i.'[lon]" value="'.$value['lon'].'" type="text" >';
                    $html .= '<label>'.esc_html__("Marker Title",'supershop').'</label><input class="st-input st-location-save st-label" name="'.$i.'[title]" value="'.$value['title'].'" type="text" >';
                    $html .= '<label>'.esc_html__("Info Box",'supershop').'</label>';
                    $html .= '<label>'.esc_html__("Info Box",'supershop').'</label><textarea id="content'.$i.'" class="st-input st-location-save info-content" name="'.$i.'[boxinfo]">'.$value['boxinfo'].'</textarea>';
                    $html .= '<a href="#" class="st-del-item">'.esc_html__("delete",'supershop').'</a>';
                $html .= '</div>';
                $i++;
            }
        }
        $html .= '</div>';
        $html .= '<div class="add-location"><button style="margin-top: 10px;padding: 5px 12px" class="vc_btn vc_btn-primary vc_btn-sm st-location-add-map" type="button">'.esc_html__('Add more', 'supershop').' </button></div>';
