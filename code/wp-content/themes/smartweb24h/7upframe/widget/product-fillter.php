...
if(!class_exists('SV_Product_Fillter') && class_exists("woocommerce"))
{
    class SV_Product_Fillter extends WP_Widget {
        ...
        function __construct() {
            ...
            parent::__construct( false, esc_html__('Product Fillter','supershop'),
                array( 'description' => esc_html__( 'Fillter product shop page', 'supershop' ), ));
            ...
            if(!is_single()){
                ...
                if(is_array($category) && !empty($category)){
                    echo        '<div class="box-filter category-filter">
                                    <h2 class="widget-title">'.esc_html__("CATEGORY","supershop").'</h2>
                                    <ul>';                
                        $cat_current = '';
                ...
                if($price == 'yes'){
                    ...
                    echo    '<div class="box-filter price-filter">
                                <h2 class="widget-title">'.esc_html__("price","supershop").'</h2>
                                <div class="inner-price-filter">
                                    <div class="range-filter">
                                        <form method="get" action="'.esc_url( $form_action ).'">
                                            ...
                                            '.$input_attr_html.'
                                            <button class="btn-filter">'.esc_html__("Filter","supershop").'</button>
        ...
        function form( $instance ) {
            ...
            <p>
                <label><?php esc_html_e( 'Categories:' ,'supershop'); ?></label></br>
                <?php 
                ...
                else echo esc_html__("No any category.","supershop");
                ?>
            </p>
            <p>
                <label for="<?php echo esc_attr($this->get_field_id( 'price' )); ?>"><?php esc_html_e( 'Filter Price:' ,'supershop'); ?></label>

                <select id="<?php echo esc_attr($this->get_field_id( 'price' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'price' )); ?>">
                    <option <?php selected('yes',$price) ?> value="yes"><?php esc_html_e("Yes","supershop")?></option>
                    <option <?php selected('no',$price) ?> value="no"><?php esc_html_e("No","supershop")?></option>

                </select>
            </p>
            <p>
                <label for="<?php echo esc_attr($this->get_field_id( 'price_min' )); ?>"><?php esc_html_e( 'Default min price:' ,'supershop'); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id( 'price_max' )); ?>"><?php esc_html_e( 'Default max price:' ,'supershop'); ?></label>
            ...
            <p>
                <label><?php esc_html_e( 'Attribute:' ,'supershop'); ?></label></br>
                <?php 
                global $wpdb;
                ...
                else echo esc_html__("No any attribute.","supershop");
                ?>
            </p>
        <?php
        }
