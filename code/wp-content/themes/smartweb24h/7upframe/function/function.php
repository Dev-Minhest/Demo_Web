...
if(!function_exists('sv_list_header_page'))
{
    function sv_list_header_page($allpage = false)
    {
        ...
        $page_list[] = array(
            'value' => '',
            'label' => esc_html__('-- Choose One --','supershop')
        );
        $args= array(
        'post_type' => 'page',
        'posts_per_page' => -1, 
        ...
        if($query->have_posts()): while ($query->have_posts()):$query->the_post();
            if ($allpage || strpos($post->post_content, '[sv_logo') ||  strpos($post->post_content, '[sv_menu') ||  strpos($post->post_content, '[sv_menu_meta')) {
                $page_list[] = array(
                    'value' => $post->ID,
...
if(!function_exists('sv_list_all_page'))
{
    function sv_list_all_page()
    {
        global $post;
        $page_list = array(
            esc_html__('-- Choose One --','supershop') => '',
            );
        $args= array(
        'post_type' => 'page',
        'posts_per_page' => -1, 
...
if(!function_exists('sv_get_order_list'))
{
    function sv_get_order_list($current=false,$extra=array(),$return='array')
    {
        $default = array(
            ...
            esc_html__('Post ID','supershop')            => 'ID',
            esc_html__('Author','supershop')             => 'author',
            esc_html__('Post Title','supershop')         => 'title',
            esc_html__('Post Name','supershop')          => 'name',
            esc_html__('Post Date','supershop')          => 'date',
            esc_html__('Last Modified Date','supershop') => 'modified',
            esc_html__('Post Parent','supershop')        => 'parent',
            esc_html__('Random','supershop')             => 'rand',
            esc_html__('Comment Count','supershop')      => 'comment_count',
            esc_html__('View Post','supershop')          => 'post_views',
            esc_html__('Like Post','supershop')          => '_post_like_count',
            esc_html__('Custom Modified Date','supershop')=> 'time_update',            
        );
...
if(!function_exists('sv_breadcrumb'))
{
    function sv_breadcrumb() {
        ...
        if (!is_home() || (is_home() && !is_front_page())) {
            ...
            echo '">';
            echo esc_html__('Home','supershop');
            echo '</a>'.' <span class="lnr lnr-chevron-right"></span> ';
            if(is_home() && !is_front_page()){
                echo '<span>'.esc_html__('Blog','supershop').'</span>'; 
            }
        ...
        elseif (is_day()) {echo"<span>".esc_html_e("Archive for ","supershop"); the_time(get_option( 'date_format' )); echo'</span>';}
        elseif (is_month()) {echo"<span>".esc_html_e("Archive for ","supershop"); echo get_the_time('F, Y'); echo'</span>';}
        elseif (is_year()) {echo"<span>".esc_html_e("Archive for ","supershop"); echo getthe_time('Y'); echo'</span>';}
        elseif (is_author()) {echo"<span>".esc_html_e("Author Archive ","supershop"); echo'</span>';}
        elseif (isset($_GET['paged']) && !empty($_GET['paged'])) {echo "<span>".esc_html_e("Blog Archives","supershop"); echo'</span>';}
        elseif (is_search()) {echo"<span>".esc_html_e("Search Results","supershop"); echo'</span>';}
    }
}
...
if(!function_exists('sv_paging_nav'))
{
    function sv_paging_nav()
    {
        ...
        $links = paginate_links( array(
            ...
            'prev_text' => '<i class="fa fa-angle-double-left"></i> '.esc_html__( 'Prev', 'supershop' ),
            'next_text' => esc_html__( 'Next', 'supershop' ).' <i class="fa fa-angle-double-right"></i>',
        ) );
        ...
        switch ($type) {
            case 'single':?>
                <ul class="post-date-author">
                    ...
                    <li><?php esc_html_e('By', 'supershop');?>: <a href="<?php echo esc_url(get_author_posts_url(get_the_author_meta('ID')));?>"><?php echo get_the_author(); ?></a></li>
                    <li><a href="<?php echo esc_url( get_comments_link() ); ?>">
                        <?php echo get_comments_number().' '; ?>
                        <?php 
                            esc_html_e('Comments', 'supershop') ;
                        ?>
                    </a></li>
                </ul>
                <?php
            ...
            default:?>
                <div class="post-tags-info blog-post-meta">
                    <label><?php esc_html_e('By', 'supershop');?>:</label>
                    <a href="<?php echo esc_url(get_author_posts_url(get_the_author_meta('ID')));?>" class="post-author-link"><?php echo get_the_author(); ?></a>
                    <label><?php esc_html_e('In', 'supershop');?>:</label>
                        <?php $cats = get_the_category_list(', ');?>
                        <?php if($cats) echo balanceTags($cats); else esc_html_e("No Category",'supershop');?>
                    
                    <?php if(is_front_page() && is_home()):?>
                    <label><?php esc_html_e('In', 'supershop');?>:</label>
                        <?php $tags = get_the_tag_list(' ',', ',' ');?>
                        <?php if($tags) echo balanceTags($tags); else esc_html_e("No Tag",'supershop');?>
                    <?php endif;?>
    ...
    function sv_get_header_default(){
        ...
        <div id="header" class="header-default">                
            <div class="header">
                <div class="container">
                    <div class="logo">
                        <a href="<?php echo esc_url(home_url('/'));?>" title="<?php echo esc_attr__("logo","supershop");?>">
                    ...
                    <nav class="main-nav">
                        ...
                        <a href="#" class="toggle-mobile-menu"><span><?php esc_html_e("Menu","supershop");?></span></a>
                    </nav>
                </div>
            </div>
        </div>
        ...
        <div id="footer" class="default-footer">
            <div class="container">
                <div class="footer-bottom">
                    <p class="copyright"><?php esc_html_e("Copyright &copy; by 7up. All Rights Reserved. Designed by","supershop")?> <a href="#"><span><?php esc_html_e("7uptheme","supershop")?></span>.<?php esc_html_e("com","supershop")?></a>.</p>
                </div>
            </div>
        </div>
        <?php
    ...
    function sv_product_thumb_hover2($size = 'full',$style = 'product-thumb3'){
        ...
        $button_html =  apply_filters( 'woocommerce_loop_add_to_cart_link',
                            sprintf( '<a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" data-quantity="%s" class="%s product_type_%s"><i class="fa fa-shopping-basket"></i> %s</a>',
                                ...
                                esc_attr( $product->get_sku() ),
                                esc_attr( isset( $quantity ) ? $quantity : 1 ),
                                $product->is_purchasable() && $product->is_in_stock() ? 'add_to_cart_button addcart-link' : 'addcart-link',
                                esc_attr( $product->get_type() ),
                                esc_html( $product->add_to_cart_text() )
                            ),
                        $product );
            ...
            switch ($style) {
                ...
                default:
                    $html = '<div class="percent-saleoff">
                                <span><label>'.$percent.'%</label>'.esc_html__('OFF','supershop').'</span>
                            </div>';
                    break;
            }            
        }
    ...
    function sv_mini_cart($echo = false){
        ...
        if ( ! WC()->cart->is_empty() ){
            ...
            $html .=    '<div class="mini-cart-total cart-qty">
                            <label>'.esc_html__("TOTAL","supershop").'</label>
                            <span class="total-price">'.WC()->cart->get_cart_total().'</span>
                        </div>
                        <div class="mini-cart-button">
                            <a href="'.WC()->cart->get_cart_url().'" class="mini-cart-view">'.esc_html__("View my cart","supershop").'</a>
                            <a href="'.WC()->cart->get_checkout_url().'" class="mini-cart-checkout">'.esc_html__("Checkout","supershop").'</a>
                        </div>';
        }
        // else $html .= '<ul class="info-list-cart"><li class="item-info-cart cart-empty"><h3>'.esc_html__("No product in cart","supershop").'</h3></li></ul>';
    ...
    function sv_product_main_detai($ajax = false){
        ...
        if(!empty($available_data)){
            foreach ($available_data as $available) {
                if(!empty($available['image_id'])){
                    ...
                    $ul_block .= '<li><a data-variation_id="'.$available['variation_id'].'" href="#">'.$image2.'</a></li>';
                    $i++;
                }
            }
        }
        ...
        if(is_array($stock)){
            if(!empty($stock['availability'])) $stock = $stock['availability'];
                ...
                if($stock['class'] == 'in-stock') $stock = esc_html__("In stock","supershop");
                else $stock = esc_html__("Out of stock","supershop");
            }
        }
                ...
                echo    '<div class="col-md-7 col-sm-12 col-xs-12">
                            <div class="detail-gallery-fullwidth">
                                <div class="mid image-lightbox" data-gallery="'.$gallerys.'">
                                    ...
                                    <p><i class="fa fa-search"></i> '.esc_html__("Mouse over to zoom in","supershop").'</p>
                                </div>
                        ...
                        <div class="col-md-5 col-sm-12 col-xs-12">
                            <div class="detail-info detail-info-fullwidth">
                                ...
                                <div class="product-order">
                                    <span>'.get_post_meta(get_the_iD(),'total_sales',true).' '.esc_html__("Orders","supershop").'</span>
                                </div>
                                '.$excerpt_html.'
                                <div class="product-code">
                                    <label>'.esc_html__("Item Code:","supershop").' </label> <span>#'.$sku.'</span>
                                </div>';
                        if($show_mode != 'on'){
                echo        '<div class="product-stock">
                                <label>'. esc_html__("Availability","supershop").': </label> <span>'.$stock.'</span>
                            </div>';
                        }
                            ...
                            <div class="row">
                                <div class="col-md-12 col-sm-12 col-xs-12">
                                    <div class="single-post-share">
                                        <label>'.esc_html__("Share","supershop").'</label>
                            ...
                            <div class="row">
                                <div class="col-md-6 col-sm-12 col-xs-12">
                                    <div class="single-post-tabs">
                                        <label>'.esc_html__("Tags:","supershop").' </label>
                                        '.$tags.'
                                    </div>
                                </div>
                                <div class="col-md-6 col-sm-12 col-xs-12">
...
if(!function_exists('sv_single_related_post')){
    ...
    ?>
        <div class="single-related-post">
            <h2 class="title"><?php esc_html_e("Relatest posts","supershop")?></h2>
                                ...
                                echo    '<div class="item">
                                            <div class="item-single-related-post">
                                                ...
                                                <div class="single-related-post-info">
                                                    ...
                                                    <a href="'. esc_url(get_the_permalink()) .'" class="readmore">'.esc_html__("Read More","supershop").'</a>
...
//Don't Show popup
if(!is_admin() && !isset($_COOKIE["PHPSESSID"])){
    session_start();
}
    ...
    function sv_product_filter_box($prices = '',$attributes = ''){
        $html = '<div class="box-product-filter">
                    <div class="list-box-filter clearfix">
                        <div class="item-box-filter">
                            <h2>'.esc_html__("Sort By","supershop").'</h2>
                            <ul>
                                <li><a class="load-data-filter" data-product_type="" href="#">'.esc_html__("Default","supershop").'</a></li>
                                <li><a class="load-data-filter" data-product_type="trendding" href="#">'.esc_html__("Trendding","supershop").'</a></li>
                                <li><a class="load-data-filter" data-product_type="toprate" href="#">'.esc_html__("Toprate","supershop").'</a></li>
                                <li><a class="load-data-filter" data-product_type="mostview" href="#">'.esc_html__("Mostview","supershop").'</a></li>
                                <li><a class="load-data-filter" data-product_type="onsale" href="#">'.esc_html__("Onsale","supershop").'</a></li>
                                <li><a class="load-data-filter" data-product_type="featured" href="#">'.esc_html__("Featured","supershop").'</a></li>
                                <li><a class="load-data-filter" data-product_type="popularity" href="#">'.esc_html__("Popularity","supershop").'</a></li>
                                <li><a class="load-data-filter" data-product_type="rating" href="#">'.esc_html__("Average rating","supershop").'</a></li>
                                <li><a class="load-data-filter" data-product_type="price-asc" href="#">'.esc_html__("Price: Low to High","supershop").'</a></li>
                                <li><a class="load-data-filter" data-product_type="price-desc" href="#">'.esc_html__("Price: High to Low","supershop").'</a></li>
                            </ul>
                        </div>';
        ...
        if(is_array($prices) && !empty($prices) && $hide_price != 'on' ){
            $html .=    '<div class="item-box-filter">
                            <h2>'.esc_html__("Price","supershop").'</h2>
                            <ul>
                                <li><a class="load-data-filter" data-price="" href="#">'.esc_html__("All","supershop").'</a></li>';
        ...
        $html .=    '<div class="item-box-filter box-tags-filter">
                            <h2>'.esc_html__("Tags","supershop").'</h2>
                            <ul>';
...
if(!function_exists('sv_scroll_top')){
    function sv_scroll_top(){
        ...
        if($scroll_top == 'on'):?>
        <a class="back-to-top" href="#"><?php esc_html_e("Top","supershop")?></a>
        <?php endif;
    }
}
        ...
        switch ($style) {
            case 'detail':
                $html = '<div class="info-price info-price-detail">
                            <label>'.esc_html__("Price:","supershop").'</label> '.$product->get_price_html().'
                        </div>';
                break;
    ...
    function sv_single_upsell_product()
    {
        ...
        if($check_show == 'on' || $check_show == 'yes'){
            ...
            <div class="upsell-detail">
                <h2 class="title-default"><?php esc_html_e("UPSELL PRODUCTS","supershop")?></h2>
...
if(!function_exists('sv_single_lastest_product'))
{
    function sv_single_lastest_product()
    {
        ...
        if($check_show == 'on' || $check_show == 'yes'){
            ...
            ?>
            <div class="upsell-detail">
                <h2 class="title-default"><?php esc_html_e("LASTEST PRODUCTS","supershop")?></h2>
                <div class="upsell-detail-slider">
                    <div class="wrap-item">
                    <?php
                        $args = array(
...
if(!function_exists('sv_single_relate_product'))
{
    ...
    {        
        ...
        if($check_show == 'on' || $check_show == 'yes'){
            ?>
            <div class="upsell-detail">
                <h2 class="title-default"><?php esc_html_e("RELATED PRODUCTS","supershop")?></h2>
                <div class="upsell-detail-slider">
                    <div class="wrap-item">
                    <?php
                        $args = array(
...
if(!function_exists('s7upf_get_icon_params')){
    function s7upf_get_icon_params($key = '',$value = ''){
        $params = array(
            array(
                'type' => 'dropdown',
                'heading' => esc_html__( 'Icon library', 'supershop' ),
                'value' => array(
                    esc_html__( 'Font Awesome', 'supershop' ) => 'fontawesome',
                    esc_html__( 'Open Iconic', 'supershop' ) => 'openiconic',
                    esc_html__( 'Typicons', 'supershop' ) => 'typicons',
                    esc_html__( 'Entypo', 'supershop' ) => 'entypo',
                    esc_html__( 'Linecons', 'supershop' ) => 'linecons',
                    esc_html__( 'Mono Social', 'supershop' ) => 'monosocial',
                ),
                'param_name' => 'type',
                'description' => esc_html__( 'Select icon library.', 'supershop' ),
                'dependency' => array(
                    'element' => $key,
                    'value' => $value,
                    )
...
if ( ! function_exists( 's7upf_catalog_ordering' ) ) {
    function s7upf_catalog_ordering($query,$set_orderby = '') {        
        ...
        $catalog_orderby_options = apply_filters( 'woocommerce_catalog_orderby', array(
            'menu_order' => __( 'Default sorting', 'supershop' ),
            'popularity' => __( 'Sort by popularity', 'supershop' ),
            'rating'     => __( 'Sort by average rating', 'supershop' ),
            'date'       => __( 'Sort by newness', 'supershop' ),
            'price'      => __( 'Sort by price: low to high', 'supershop' ),
            'price-desc' => __( 'Sort by price: high to low', 'supershop' )
        ) );
...
if(!function_exists('s7upf_tool_panel')){
    function s7upf_tool_panel($content){
