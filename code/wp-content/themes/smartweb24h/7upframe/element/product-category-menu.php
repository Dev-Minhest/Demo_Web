...
if(class_exists("woocommerce")){
    if(!function_exists('sv_vc_category_menu'))
    {
        function sv_vc_category_menu($attr, $content = false)
        {
            ...
            extract(shortcode_atts(array(
                'style'      => 'load-hidden',
                'home_style' => 'right-category-dropdown',
                'title'      => '',
                'expand'     => '9',
            ),$attr));                
            ...
            $html .=            '</ul>';
            if($home_style == 'category-dropdown8' || $home_style == 'category-dropdown8 category-dropdown9') $html .=  '<a class="expand-category-link expand-category-link8" href="#">'.esc_html__("All Categories","supershop").'</a>';
            else $html .=            '<a href="#" class="expand-category-link">
                                        <span class="more-text">'.esc_html__("More","supershop").'</span>
                                        <span class="less-text">'.esc_html__("Less","supershop").'</span>
                                    </a>';
            $html .=         '</div>';
            $html .=    '</div>';
            return $html;
    ...
    vc_map(
        array(
            'name'     => esc_html__( 'Product Category Menu', 'supershop' ),
            ...
            'params'   => array(
                array(
                    ...
                    'param_name'  => 'style',
                    'value'       => array(
                        esc_html__( 'Default', 'supershop' )  => 'load-hidden',
                        esc_html__( 'Active', 'supershop' )   => 'load-active',
                        ),
                ),
                array(
                    'type'        => 'dropdown',
                    'heading'     => esc_html__( 'Home Style', 'supershop' ),
                    'param_name'  => 'home_style',
                    'value'       => array(
                        esc_html__( 'Default(right)', 'supershop' )  => 'right-category-dropdown',
                        esc_html__( 'Default(left)', 'supershop' )  => 'left-category-dropdown',
                        esc_html__( 'Home 4', 'supershop' )  => 'wrap-category-hover4',
                        ),
                ),
                array(
                    'type'        => 'textfield',
                ...
                array(
                    'type'        => 'textfield',
                    'heading'     => esc_html__( 'Expand Number', 'supershop' ),
                    'param_name'  => 'expand',
                ),
            )
        )
                ...
                switch ($mega_content) {
                    ...
                    case 'featured':
                        $content_hover .=       '<div class="cat-mega-menu cat-mega-style2">
                                                    <h2 class="title-cat-mega-menu">'.esc_html__("Special products","supershop").'</h2>
                                                    <div class="row">';
                        $args = array(
                            'post_type'         => 'product',
                            'posts_per_page'    => 3,
...
if(empty($check_add)) add_action( 'vc_before_init_base','sv_admin_category_item',10,100 );
    if ( ! function_exists( 'sv_admin_category_item' ) ) {
        function sv_admin_category_item(){
            vc_map(
                array(
                    'name'     => esc_html__( 'Category Item', 'supershop' ),
                    ...
                    'params'   => array(
                        ...
                        array(
                            'type'        => 'attach_image',
                            'heading'     => esc_html__( 'Icon image', 'supershop' ),
                            'param_name'  => 'icon',
                        ),
                        array(
                            'type'        => 'dropdown',
                            'heading'     => esc_html__( 'Content hover', 'supershop' ),
                            'param_name'  => 'mega_content',
                            'value'       => array(
                                esc_html__( 'None', 'supershop' )             => '',
                                esc_html__( 'Content page', 'supershop' )     => 'page',
                                esc_html__( 'Content Editor', 'supershop' )   => 'editor',
                                esc_html__( 'Featured Product', 'supershop' ) => 'featured',
                                ),
                        ),
                        array(
                            'type'        => 'dropdown',
                            'heading'     => esc_html__( 'Choose page content', 'supershop' ),
                            'param_name'  => 'page_id',
